...
function SettingsCtrl($rootScope, $scope, $state, $location, $window, $http) {
  ...
  $http.get(URL_STATUS_GET).then(function (response) {
    ...
    else $scope.hasOgnTracker = false;
  });
  ...
  function loadSettings(data) {
    ...
    if (
      settings.SerialOutputs !== undefined &&
      settings.SerialOutputs !== null
    ) {
    ...
    $scope.WiFiInternetPassThroughEnabled =
      settings.WiFiInternetPassThroughEnabled;
  ...
  function getSettings() {
    ...
    $http.get(URL_SETTINGS_GET).then(
      function (response) {
  ...
  function setSettings(msg) {
    ...
    $http.post(URL_SETTINGS_SET, msg).then(
      function (response) {
        loadSettings(response.data);
        // $scope.$apply();
      },
  ...
  $scope.updatestaticips = function () {
    if ($scope.StaticIps !== settings.StaticIps) {
      var newsettings = {
        StaticIps:
          $scope.StaticIps === undefined ? "" : $scope.StaticIps.join(" "),
      };
      // console.log(angular.toJson(newsettings));
      setSettings(angular.toJson(newsettings));
    }
  ...
  $scope.updatealtitudeoffset = function () {
    if (
      $scope.AltitudeOffset !== undefined &&
      $scope.AltitudeOffset !== null &&
      $scope.AltitudeOffset !== settings["AltitudeOffset"]
    ) {
  ...
  $scope.uploadFile = function () {
    ...
    $http
      ...
      .success(function (data) {
        ...
        alert(
          "success. wait 5 minutes and refresh home page to verify new version."
        );
  ...
  $scope.setOrientation = function (action) {
    ...
    $http.post(URL_AHRS_ORIENT, action).then(
      function (response) {
  ...
  $scope.updateWiFi = function (action) {
    ...
    if (
      $scope.WiFiSSID === undefined ||
      $scope.WiFiSSID === null ||
      !isValidSSID($scope.WiFiSSID)
    ) {
      $scope.WiFiErrors.WiFiSSID =
        'Your Network Name("SSID") must be at least 1 character ' +
    ...
    if ($scope.WiFiSecurityEnabled) {
      if (!isValidWPA($scope.WiFiPassphrase)) {
        $scope.WiFiErrors.WiFiPassphrase =
          "Your WiFi Password, " +
          $scope.WiFiPassphrase +
          ", contains invalid characters.";
        $scope.WiFiErrors.Errors = true;
      }
      if (
        $scope.WiFiPassphrase.length < 8 ||
        $scope.WiFiPassphrase.length > 63
      ) {
        $scope.WiFiErrors.WiFiPassphrase =
          "Your WiFi Password must be between 8 and 63 characters long.";
        $scope.WiFiErrors.Errors = true;
      }
    }
...
function isValidSSID(str) {
  return /^[a-zA-Z0-9()! \._-]{1,32}$/g.test(str);
}
function isValidWPA(str) {
  return /^[\u0020-\u007e]{8,63}$/g.test(str);
}
function isValidPin(str) {
  return /^([\d]{4}|[\d]{8})$/g.test(str);
}

angular
  .module("appControllers")
  .directive("hexInput", function () {
    // directive for ownship hex code validation
    return {
      require: "ngModel",
  ...
  .directive("watchlistInput", function () {
    // directive for ICAO space-separated watch list validation
    return {
      require: "ngModel",
      link: function (scope, element, attr, ctrl) {
        function watchListValidation(value) {
          ...
          var r = "[A-Za-z0-9]{4}";
          var valid = new RegExp("^(" + r + "( " + r + ")*|)$", "g").test(
            value
          );
  ...
  .directive("ssidInput", function () {
    // directive for WiFi SSID validation
    return {
      require: "ngModel",
      link: function (scope, element, attr, ctrl) {
  ...
  .directive("wpaInput", function () {
    // directive for WiFi WPA Passphrase validation
    return {
      require: "ngModel",
      link: function (scope, element, attr, ctrl) {
  ...
  .directive("ipListInput", function () {
    // directive for validation of list of IP addresses
    return {
      require: "ngModel",
      link: function (scope, element, attr, ctrl) {
  ...
  .directive("ipAddrInput", function () {
    // directive for validation of a single IP address
    return {
      require: "ngModel",
      link: function (scope, element, attr, ctrl) {
        function ipListValidation(value) {
  ...
  .directive("gLimitsInput", function () {
    // directive for validation of list of G Limits
    return {
      require: "ngModel",
      link: function (scope, element, attr, ctrl) {
